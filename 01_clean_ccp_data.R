...
ccp_data <- ccp_data %>% 
  ...
  filter(daily_neutro_lborres <= 50)
...
ccp_data_clean <- ccp_data
###################################################################### 
### Save the cleaned CCP_data file
save(ccp_data_clean, file = 
       "/home/u034/mfourman/ccp_data_clean.rda")

load("/home/u034/mfourman/ccp_data_clean.rda")

###################################################################### 
## Make topline (topline is day 1 data only)

# First get topline
topline = ccp_data_clean %>% 
...
save(topline, file = 
       "/home/u034/mfourman/topline.rda")

topline %>% select(
  "age", "sex", "ethnicity", "onset2admission", "any_icu", "outcome") %>% 
  ff_glimpse()



# Create a smaller table whilst working on these graphs
ccp_blood_data <- ccp_data_clean %>%
  select(subjid, cestdat, daily_lbdat, onset_to_lab, admission_to_lab, 
         daily_wbc_lborres, daily_lymp_lborres, 
         daily_plt_lborres, daily_neutro_lborres, 
         daily_nlr_lborres, severity2, outcome, dsterm, dsstdtc)



#pivot longer for easier graphing
ccp_blood_data_long <- ccp_blood_data %>%
  pivot_longer(c(daily_wbc_lborres, daily_lymp_lborres, 
                 daily_plt_lborres, daily_neutro_lborres, daily_nlr_lborres), 
               names_to = "assay", 
               values_to = "value")



### Now save this to make life easier
save(ccp_blood_data_long, file = 
       "/home/u034/mfourman/ccp_blood_data_long.rda")
